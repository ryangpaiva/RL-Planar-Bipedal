...
class BipedalEnv(MujocoEnv, utils.EzPickle):
    """
    ### Description
        ...
        **kwargs
        ...
        if exclude_current_positions_from_observation:
            observation_space = Box(
                low=-np.inf, high=np.inf, shape=(13,), dtype=np.float64 #change the number of observations here
            )
        else:
            observation_space = Box(
                low=-np.inf, high=np.inf, shape=(13,), dtype=np.float64  #change the number of observations here
        ...
        MujocoEnv.__init__(
            self, "Robot_Assembly_straight_knee_feet.xml", 4, observation_space=observation_space, **kwargs
        )
